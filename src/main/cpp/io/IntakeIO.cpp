...
bool IntakeIO::ProcessIO() {
    ...
    sw_interface_->intake_position_encoder_val =
        hw_interface_->intake_position_motor->GetEncoder().GetPosition();
...
bool IntakeIO::UpdateIntakeConfig(IntakeConfig &config) {
    ...
    // Open Loop Ramp Rate
    hw_interface_->intake_motor->SetOpenLoopRampRate(open_loop_ramp);
...
bool IntakeIO::ResetEncoders() {
    ...
    hw_interface_->intake_position_motor->GetEncoder().SetPosition(0.0);
